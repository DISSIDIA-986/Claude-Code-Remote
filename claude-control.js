...
class RemoteControlSetup {
  ...
  findClaudeCodeRemoteHome() {
    ...
    if (fs.existsSync(path.join(__dirname, 'package.json'))) {
      ...
      try {
        ...
        if (
          packageJson.name &&
          packageJson.name.toLowerCase().includes('claude-code-remote')
        ) {
          return __dirname;
        }
      } catch (e) {
  ...
  async setup() {
    ...
    console.log(
      '🎯 Goal: Remote access via mobile phone → Home computer Claude Code automatically executes commands\n'
        ...
        if (error) {
          ...
          exec(
            ...
            (installError, installStdout, installStderr) => {
              if (installError) {
                console.log(
                  '❌ tmux installation failed, please install manually:'
                );
  ...
  createNewSession(resolve) {
    ...
    console.log(
      `💡 Using convenience command: clauderun (equivalent to claude --dangerously-skip-permissions)`
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        ...
        exec(fallbackCommand, fallbackError => {
          if (fallbackError) {
            console.log(
              `❌ Full path command also failed: ${fallbackError.message}`
            );
          } else {
            console.log(
              '✅ Claude tmux session created successfully (using full path)'
            );
  ...
  async testRemoteInjection() {
    ...
    console.log(
      '🔧 To test injection functionality, please use separate test script'
    );
  ...
  showUsageGuide() {
    ...
    console.log(
      '   You can now use clauderun instead of claude --dangerously-skip-permissions'
    );
    ...
    console.log(
      '3. 📱 Receive Claude-Code-Remote email notifications on mobile'
    );
    console.log('4. 💬 Reply to email with commands on mobile');
    console.log(
      '5. 🤖 Claude at home automatically receives and executes commands'
    );
    ...
    console.log(
      '   Create custom session: node claude-control.js --session my-project'
    );
    console.log(
      '   Create multiple sessions: node claude-control.js --session frontend'
    );
    console.log('                    node claude-control.js --session backend');
    console.log(
      '   Email replies will automatically route to corresponding session\n'


    console.log('📱 Email testing:');
    console.log(
      '   Token will include session information, automatically routing to correct tmux session'
    );
    ...
    console.log(
      "- Claude session runs continuously in tmux, won't be interrupted by network disconnection/reconnection"
    );
    console.log('- Email monitoring service needs to remain running');
    console.log(
      '- Home computer needs to stay powered on with network connection'
    );
    console.log('- Mobile can send email commands from anywhere');
    console.log(
      '- Supports running multiple Claude sessions for different projects simultaneously\n'
