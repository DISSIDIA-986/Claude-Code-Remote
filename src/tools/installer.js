...
class Installer {
  ...
  checkDependencies() {
    ...
    switch (platform) {
      ...
      case 'linux':
        console.log(
          'ℹ️  Linux system, please ensure libnotify-bin is installed'
        );
        break;
      case 'win32':
  ...
  async installHooks() {
    ...
    if (!fs.existsSync(this.claudeConfigDir)) {
      ...
      console.log(
        `✅ Created configuration directory: ${this.claudeConfigDir}`
      );
    }
    ...
    if (fs.existsSync(settingsPath)) {
      try {
        ...
        console.log(
          '⚠️  Unable to parse existing settings, will create new configuration'
        );
      }
    }
  ...
  displayUsage() {
    ...
    console.log(
      '• You will receive reminders when Claude is waiting for input'
    );
    console.log('');
    console.log('Common commands:');
    console.log(
      `  node "${path.join(this.projectDir, 'claude-remote.js')}" config`
    );
    console.log(
      `  node "${path.join(this.projectDir, 'claude-remote.js')}" test`
    );
    console.log(
      `  node "${path.join(this.projectDir, 'claude-remote.js')}" status`
    );
    console.log('');
    console.log(
      'To uninstall, manually delete the hooks configuration from Claude Code settings.'
    );
  }
}
