...
class ConfigurationManager {
  ...
  displayCurrentConfig() {
    ...
    console.log(
      '├─ Enabled status:',
      this.config.get('enabled') ? 'Enabled' : 'Disabled'
    );
  ...
  async configureLanguage() {
    ...
    const choice = await this.question(
      `Select language (1-${languages.length}): `
    );
  ...
  async configureSounds() {
    ...
    const completedSound = await this.selectSoundFromCategories(
      soundCategories,
      'task completion'
    );
    if (completedSound) {
    ...
    const waitingSound = await this.selectSoundFromCategories(
      soundCategories,
      'waiting input'
    );
    if (waitingSound) {
  ...
  async configureRelay() {
    ...
    console.log(
      'Will support sending commands via notification channels and auto-executing in Claude Code'
  ...
  async configureCustomMessages() {
    ...
    const currentCompleted =
      this.config.get('customMessages.completed') || 'Use default text';
    console.log(`Current task completion text: ${currentCompleted}`);
    const completedMsg = await this.question(
      'New task completion text (Enter to skip): '
    );
    ...
    const currentWaiting =
      this.config.get('customMessages.waiting') || 'Use default text';
    console.log(`\nCurrent waiting input text: ${currentWaiting}`);
    const waitingMsg = await this.question(
      'New waiting input text (Enter to skip): '
    );
    if (waitingMsg.trim()) {
  ...
  async configureEmailChannel() {
    ...
    const currentEmailConfig = this.config.getChannel('email') || {
      enabled: false,
      config: {},
    };
    ...
    console.log(
      `Current status: ${
        currentEmailConfig.enabled ? '✅ Enabled' : '❌ Disabled'
    ...
    console.log(
      `Current secure connection: ${currentSecure ? 'SSL/TLS' : 'STARTTLS'}`
    );
    ...
    const imapSecureInput = await this.question(
      'IMAP use SSL? (y/n, default y): '
    );
    ...
    const fromEmail = await this.question(
      `Sender display name (default: Claude-Code-Remote <${smtpUser}>): `
  ...
  async testEmailChannel() {
    ...
    try {
      ...
      if (testResult) {
        ...
        console.log(
          '📧 Please check your inbox, you should receive a test email'
        );
        console.log(
          '💡 You can try replying to that email to test the command relay feature'
        );
      } else {
      ...
      console.log(
        'Please check your network connection and email configuration'
      );
    }
  }
