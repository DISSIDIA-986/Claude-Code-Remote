...
function cleanEmailText(text = '') {
  ...
  for (const line of contentLines) {
    ...
    if (
      trimmedLine.includes('Claude-Code-Remote Notification System') ||
...
function deduplicateCommand(command) {
  ...
  for (let i = 1; i <= Math.floor(length / 2); i++) {
    ...
    if (
      remaining ===
      firstPart.repeat(Math.floor(remaining.length / firstPart.length))
    ) {
...
async function injectCommandRemote(token, command) {
  ...
  try {
    ...
    if (tmuxResult.success) {
      log.info(
        { token, session: tmuxResult.session },
  ...
  return new Promise(resolve => {
    ...
    pbcopy.on('close', code => {
      ...
      const notificationScript = `
                display notification "🚨 Email command auto-copied! Please paste and execute in Claude Code immediately (Cmd+V)" with title "TaskPing Auto-Injection" subtitle "${shortCommand.replace(
                  /"/g,
                  '\\"'
                )}" sound name "Basso"
            `;
...
async function handleMailMessage(parsed) {
  try {
    log.debug(
      { uid: parsed.uid, messageId: parsed.messageId },
      'handleMailMessage called'
    );
    ...
    if (!uid) {
      ...
      if (identifier && PROCESSED_MESSAGES.has(identifier)) {
        log.debug(
          ...
          'Message already processed by messageId, skipping'
        );
        return;
      }
      ...
      if (PROCESSED_MESSAGES.has(contentHash)) {
        log.debug(
          { subject: emailSubject, date: emailDate, contentHash },
          'Message already processed by content hash, skipping'
        );
        return;
      }
    }
...
function startImap() {
  ...
  imap.once('ready', function () {
    ...
    imap.openBox('INBOX', false, function (err, box) {
      ...
      log.info(
        `Mailbox opened: ${box.messages.total} total messages, ${box.messages.new} new`
  ...
  fetch.on('message', function (msg, seqno) {
    ...
    msg.once('attributes', function (attrs) {
      ...
      if (messageUid && PROCESSED_MESSAGES.has(messageUid)) {
        log.debug(
          { uid: messageUid, seqno },
          'Message UID already processed, skipping entire message'
        );
        skipProcessing = true;
      ...
      if (buffer.length > 0 && uid) {
        ...
        simpleParser(buffer, function (err, parsed) {
          if (err) {
            ...
            log.debug(
              { uid, seqno },
              'Email parsed successfully, calling handleMailMessage'
            );
            parsed.uid = uid;
        ...
        log.debug(
          { uid, seqno, bufferLength: buffer.length },
          'Skipping email - no buffer or uid'
        );
      }
    ...
    msg.once('error', function (err) {
      log.error(
        { error: err.message, seqno, uid: messageUid },
