        ...
        this.copyToClipboard(filePath).then(() => {
          ...
          exec(`osascript -e '${notificationScript}'`, () => {
            ...
            exec(`open "${this.tempDir}"`, () => {
              resolve({
                success: true,
                message: 'File created, notification sent',
              });
            });
          });
          ...
          const notifications = [
            {
              ...
              sound: 'Basso',
              message:
                '🚨 Email command copied! Please paste immediately to Claude Code (Cmd+V)',
            },
            {
              delay: 3000,
              sound: 'Ping',
              message:
                '⏰ Reminder: Command still in clipboard, please paste and execute',
            },
            {
              delay: 8000,
              sound: 'Purr',
              message:
                '💡 Final reminder: Press Cmd+V in Claude Code to paste command',
            },
            ...
            setTimeout(() => {
              const script = `
                            display notification "${
                              notif.message
                            }" with title "TaskPing Persistent Reminder ${
                index + 1
              }/3" subtitle "${command.slice(0, 30)}..." sound name "${
                notif.sound
              }"
              ...
              exec(`osascript -e '${script}'`, () => {
                ...
                if (completedNotifications === notifications.length) {
                  resolve({
                    success: true,
                    message: 'Persistent notification sequence completed',
                  });
                }
              });
            }, notif.delay);
...
`;
          ...
          try {
            ...
            const script = `
                        display notification "🆘 Emergency Mode: Desktop shortcut file TaskPing-${token}.command created\\nDouble-click to re-copy command to clipboard" with title "TaskPing Emergency Mode" subtitle "Command: ${command.slice(
              0,
              20
            )}..." sound name "Sosumi"
                    `;
            ...
            exec(`osascript -e '${script}'`, () => {
              resolve({
                success: true,
                message: 'Emergency mode: Desktop shortcut file created',
              });
