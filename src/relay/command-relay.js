...
class CommandRelayService extends EventEmitter {
  ...
  async start() {
    ...
    try {
      ...
      this.logger.error(
        'Failed to start command relay service:',
        error.message
      );
      throw error;
    }
  ...
  async _executeCommand(commandItem) {
    ...
    try {
      ...
      const success = await this._sendCommandToClaudeCode(
        commandItem.command,
        claudeProcess,
        commandItem.sessionId
      ...
      if (success) {
        ...
        if (this.emailListener) {
          await this.emailListener.updateSessionCommandCount(
            commandItem.sessionId
          );
        }
  ...
  async _findClaudeCodeProcess() {
    return new Promise(resolve => {
      ...
      ps.on('close', code => {
        ...
        const claudeProcesses = lines.filter(
          line =>
        ...
        if (claudeProcesses.length > 0) {
          ...
          this.logger.debug(
            'No Claude Code process found, will try desktop automation'
          );
  ...
  async _sendCommandToClaudeCode(command, claudeProcess, sessionId) {
    return new Promise(async resolve => {
      try {
        ...
        const claudeSuccess = await this.claudeAutomation.sendCommand(
          command,
          sessionId
        ...
        if (claudeSuccess) {
          this.logger.info(
            'Command sent and executed successfully via Claude automation'
          );
          resolve(true);
          return;
        }
        ...
        if (this.clipboardAutomation.isSupported()) {
          this.logger.info(
            'Attempting to send command via clipboard automation...'
          );
          const clipboardSuccess = await this.clipboardAutomation.sendCommand(

          );

          if (clipboardSuccess) {
            this.logger.info(
              'Command sent successfully via clipboard automation'
            );
            resolve(true);
            return;
          }
          this.logger.warn(
            'Clipboard automation failed, trying other methods...'
          );
        ...
        const simpleSuccess = await this.simpleAutomation.sendCommand(
          command,
        ...
        const bridgeSuccess = await this.commandBridge.sendCommand(
          command,
          sessionId
        );
        ...
        this.logger.error(
          'Error sending command to Claude Code:',
          error.message
        );
        resolve(false);
      }
    ...
    return new Promise(resolve => {
      try {
        ...
        if (process.platform === 'darwin') {
          ...
          const script = `
                        display notification "Command: ${commandPreview.replace(
                          /"/g,
                          '\\"'
                        )}" with title "TaskPing - Email Command" subtitle "Click Terminal or Claude Code window, then paste command" sound name "default"
                    `;
  ...
  _handleCommandError(commandItem, error) {
    ...
    if (commandItem.retries < commandItem.maxRetries) {
      ...
      commandItem.retryAt = new Date(
        Date.now() + commandItem.retries * 60000
      ).toISOString(); // Delayed retry
      this.logger.info(
        `Command ${commandItem.id} will be retried (attempt ${
          commandItem.retries + 1
        })`
      );
    } else {
      ...
      commandItem.status = 'failed';
      this.logger.error(
        `Command ${commandItem.id} failed after ${commandItem.retries} retries`
      );
    }
