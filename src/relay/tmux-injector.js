...
class TmuxInjector {
  ...
  async createClaudeSession() {
    return new Promise(resolve => {
      ...
      const command = `tmux new-session -d -s ${
        this.sessionName
      } -c "${process.cwd()}" clauderun`;
      ...
      exec(command, (error, stdout, stderr) => {
        if (error) {
          this.log.warn(
            `Failed to create tmux session with clauderun: ${error.message}`
          );
          // If clauderun fails, try using full path command
          this.log.info('Fallback to full path command...');
          const fallbackCommand = `tmux new-session -d -s ${
            this.sessionName
          } -c "${process.cwd()}" /Users/jessytsui/.nvm/versions/node/v18.17.0/bin/claude --dangerously-skip-permissions`;

          exec(fallbackCommand, fallbackError => {
            if (fallbackError) {
              this.log.error(
                `Failed to create tmux session with fallback: ${fallbackError.message}`
              );
              resolve({ success: false, error: fallbackError.message });
            } else {
              this.log.info(
                'Tmux Claude session created successfully (full path)'
              );
              setTimeout(() => {
            ...
            exec(sendCommand, sendError => {
              ...
              setTimeout(() => {
                exec(enterCommand, async enterError => {
                  if (enterError) {
                    this.log.error(
                      `Failed to send enter: ${enterError.message}`
                    );
  ...
  async handleConfirmations() {
    ...
    while (attempts < maxAttempts) {
      ...
      this.log.info(
        `Confirmation check ${attempts}: ${output
          .slice(-200)
          .replace(/\n/g, ' ')}`
      );
                ...
                exec(
                  ...
                  enterError => {
                    if (enterError) {
                      ...
                      this.log.info(
                        'Enter sent after option 2 - no future dialogs'
                      );
                    }
                    resolve();
                  }
      ...
      if (
        ...
        this.log.info(
          `Detected Enter prompt, sending Enter (attempt ${attempts})`
      ...
      if (
        ...
        this.log.warn(
          'Detected error in output, stopping confirmation attempts'
        );
        break;
      }
    ...
    if (finalCapture.success) {
      this.log.info(
        `Final state: ${finalCapture.output.slice(-100).replace(/\n/g, ' ')}`
      );
    }
    ...
    try {
      ...
      if (!tmuxAvailable) {
        return {
          ...
          message: 'Need to install tmux: brew install tmux',
        };
      }
  ...
  async sendSuccessNotification(command) {
    ...
    const notificationScript = `
            display notification "🎉 Command automatically injected into Claude! No manual operation needed" with title "TaskPing Remote Control Success" subtitle "${shortCommand.replace(
              /"/g,
              '\\"'
            )}" sound name "Glass"
        `;
