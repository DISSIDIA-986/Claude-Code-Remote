...
class EmailListener extends EventEmitter {
  ...
  async _searchAndProcessMails() {
    return new Promise((resolve, reject) => {
      ...
      const searchCriteria = ['UNSEEN', ['SINCE', this.lastCheckTime]];
  ...
  _extractSessionId(email) {
    ...
    const htmlSessionMatch = html.match(
      /Session ID:\s*<code>([a-f0-9-]{36})<\/code>/i
    );
    if (htmlSessionMatch) {
  ...
  async updateSessionCommandCount(sessionId) {
    ...
    if (fs.existsSync(sessionFile)) {
      try {
        ...
        this.logger.debug(
          `Updated command count for session ${sessionId}: ${sessionData.commandCount}`
        );
      } catch (error) {
        this.logger.error(
          `Error updating session ${sessionId}:`,
          error.message
        );
      }
    }
