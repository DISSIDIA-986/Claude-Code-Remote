  ...
  _sendMacOS(title, message, sound) {
    try {
      ...
      try {
        ...
        execSync(cmd, {
          timeout: parseInt(process.env.NOTIFICATION_TIMEOUT) || 3000,
        });
        return true;
      } catch (e) {
  ...
  _sendLinux(title, message, sound) {
    try {
      const notificationTimeout =
        parseInt(process.env.NOTIFICATION_TIMEOUT) || 3000;
      const displayTime =
        parseInt(process.env.NOTIFICATION_DISPLAY_TIME) || 10000;
      execSync(`notify-send "${title}" "${message}" -t ${displayTime}`, {
