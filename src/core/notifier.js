...
class Notifier {
  ...
  async notify(type, metadata = {}) {
    ...
    this.logger.info(
      `Sending ${type} notification for project: ${notification.project}`
    );
    ...
    this.logger.info(
      `Notification sent to ${successCount}/${this.channels.size} channels`
    );
  ...
  _buildNotification(type, metadata = {}) {
    ...
    const finalMessage = customMessage
      ? customMessage.replace('{project}', project)
      : message;
    ...
    this.i18n = {
      'zh-CN': {
        completed: {
          ...
          message:
            '[{project}] Task completed, Claude is waiting for next instruction',
        },
        waiting: {
      ...
      ja: {
        completed: {
          ...
          message:
            '[{project}] タスクが完了しました。Claudeが次の指示を待っています',
        },
        waiting: {
