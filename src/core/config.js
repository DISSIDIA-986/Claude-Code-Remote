...
class ConfigManager {
      ...
      email: {
        ...
        config: {
          ...
          from:
            ...
            `${process.env.EMAIL_FROM_NAME || 'Claude Code Remote'} <${
  ...
  load() {
    ...
    try {
      if (fs.existsSync(this.defaultConfigPath)) {
        const fileConfig = JSON.parse(
          fs.readFileSync(this.defaultConfigPath, 'utf8')
        );
    ...
    try {
      if (fs.existsSync(this.channelsConfigPath)) {
        const fileChannels = JSON.parse(
          fs.readFileSync(this.channelsConfigPath, 'utf8')
        );
  ...
  save() {
    ...
    try {
      ...
      fs.writeFileSync(
        this.channelsConfigPath,
        JSON.stringify(this._channels, null, 2)
      );
  ...
  getProjectName() {
    try {
      ...
      const gitName = execSync('git rev-parse --show-toplevel 2>/dev/null', {
        encoding: 'utf8',
      }).trim();
      if (gitName) {
