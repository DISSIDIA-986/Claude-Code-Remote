...
class TmuxMonitor {
  ...
  startCapture(sessionName) {
    try {
      ...
      console.error(
        `Failed to start capture for session ${sessionName}:`,
        error.message
      );
      return null;
    }
  ...
  stopCapture(sessionName) {
    try {
      ...
      console.error(
        `Failed to stop capture for session ${sessionName}:`,
        error.message
      );
    }
  }
  ...
  getRecentConversation(sessionName, lines = 50) {
    try {
      ...
      console.error(
        `Failed to get conversation for session ${sessionName}:`,
        error.message
      );
  ...
  getFromTmuxBuffer(sessionName, lines = 50) {
    try {
      ...
      console.error(
        `Failed to get tmux buffer for session ${sessionName}:`,
        error.message
      );
      return { userQuestion: '', claudeResponse: '' };
    }
