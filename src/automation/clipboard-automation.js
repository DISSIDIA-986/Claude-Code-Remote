  ...
  async _copyToClipboard(text) {
    return new Promise((resolve, reject) => {
      if (process.platform === 'darwin') {
          ...
          if (code === 0) {
            ...
            xsel.on('close', code => {
              if (code === 0) {
                ...
                reject(
                  new Error(
                    'Failed to copy to clipboard (xclip/xsel not available)'
                  )
                );
              }
            });
        ...
        reject(
          new Error('Clipboard automation not supported on this platform')
        );
      }
    });
  }
    ...
    return new Promise(resolve => {
      ...
      osascript.on('close', code => {
        if (code === 0 && output === 'success') {
          ...
          this.logger.warn('AppleScript execution result:', {
            code,
            output,
            error,
          });
