...
class SimpleAutomation {
  ...
  async sendCommand(command, sessionId = '') {
    try {
      ...
      const notificationSuccess = await this._sendRichNotification(
        command,
        sessionId
      );
...
`;
  ...
  async _sendRichNotification(command, sessionId) {
    try {
      if (process.platform === 'darwin') {
        ...
        const script = `
                    set commandText to "${command
                      .replace(/"/g, '\\"')
                      .replace(/\n/g, '\\n')}"
                    
                    display notification "Command copied to clipboard, please paste to Claude Code" with title "TaskPing - New Email Command" subtitle "${shortCommand.replace(
                      /"/g,
                      '\\"'
                    )}" sound name "default"
        ...
        return new Promise(resolve => {
          osascript.on('close', code => {
            if (code === 0) {
              ...
              this.logger.warn(
                'Rich notification failed, trying simple notification'
              );
  ...
  async _sendSimpleNotification(command) {
    try {
      const shortCommand =
        command.length > 50 ? command.substring(0, 50) + '...' : command;
      const script = `display notification "Command: ${shortCommand.replace(
        /"/g,
        '\\"'
      )}" with title "TaskPing - Email Command" sound name "default"`;
