...
class ClaudeCodeRemoteCLI {
  ...
  async handleStatus(args) {
    ...
    console.log(
      `  Sounds: ${status.config.sound.completed} / ${status.config.sound.waiting}`
    );
  ...
  async startRelay(args) {
    try {
      ...
      relayService.on('started', () => {
        ...
        console.log(
          '💡 You can now remotely execute Claude Code commands by replying to emails'
        );
        console.log('');
      ...
      relayService.on('commandQueued', command => {
        console.log(
          `📨 Received new command: ${command.command.substring(0, 50)}...`
        );
      });
      ...
      relayService.on('commandFailed', (command, error) => {
        console.log(
          `❌ Command execution failed: ${command.id} - ${error.message}`
        );
      });
  ...
  async stopRelay(args) {
    ...
    console.log(
      'If the service is still running, please find the corresponding process and terminate it manually'
    );
  }
  ...
  async relayStatus(args) {
    try {
      ...
      console.log(
        `📧 SMTP: ${emailConfig.config.smtp.host}:${emailConfig.config.smtp.port}`
      );
      console.log(
        `📥 IMAP: ${emailConfig.config.imap.host}:${emailConfig.config.imap.port}`
      );
          ...
          state.commandQueue.slice(-5).forEach(cmd => {
            const status =
              cmd.status === 'completed'
                ...
                : '⏸️';
            console.log(
              `  ${status} ${cmd.id}: ${cmd.command.substring(0, 50)}...`
            );
          });
        }
      } else {
  ...
  async handleEditConfig(args) {
    ...
    if (!configType) {
      ...
      console.log(
        '  user      - User personal configuration (config/user.json)'
      );
      console.log(
        '  channels  - Notification channel configuration (config/channels.json)'
      );
      console.log(
        '  default   - Default configuration template (config/default.json)'
      );
      console.log('');
    ...
    const editor =
      process.env.EDITOR || process.env.VISUAL || this._getDefaultEditor();
    ...
    try {
      ...
      editorProcess.on('close', code => {
        if (code === 0) {
          ...
          console.log(
            '💡 Run "claude-remote status" to view updated configuration'
          );
        } else {
    ...
    try {
      ...
      switch (providerChoice) {
        case '1':
          ...
          console.log('\n📧 Gmail Configuration');
          console.log(
            '💡 Need to enable two-factor authentication and generate app password first'
          );
          break;
        case '2':
        ...
        case '5':
          ...
          smtpPort = parseInt(
            (await question('SMTP Port (default 587): ')) || '587'
          );
          imapHost = await question('IMAP Host: ');
          imapPort = parseInt(
            (await question('IMAP Port (default 993): ')) || '993'
          );
  ...
  async handleCommands(args) {
    ...
    switch (command) {
      ...
      default:
        console.log(
          'Usage: claude-remote commands <list|status|cleanup|clear>'
        );
        console.log('');
        console.log('Commands:');
  ...
  async handleTestPaste(args) {
    ...
    const testCommand =
      args.join(' ') || 'echo "Testing email reply auto-paste functionality"';
    ...
    try {
      ...
      if (success) {
        ...
        console.log(
          "💡 If you don't see the effect, please check app permissions and window status"
        );
      } else {
        console.log('❌ Auto-paste failed');
        console.log(
          '💡 Please ensure automation permissions are granted and target app is open'
        );
      }
    } catch (error) {
  ...
  async handleTestSimple(args) {
    ...
    const testCommand =
      args.join(' ') || 'echo "Testing simple automation functionality"';
    ...
    try {
      ...
      if (success) {
        ...
        console.log(
          '3. If you see dialog box, you can choose to open command file'
        );
        console.log(
  ...
  async handleTestClaude(args) {
    ...
    const testCommand =
      args.join(' ') ||
      'echo "This is an automated test command from email reply"';
    ...
    try {
      ...
      if (success) {
        ...
        console.log(
          '💡 Command should have been automatically input to Claude Code and started execution'
        );
        console.log(
          '🔍 Please check Claude Code window to see if command was received'
        );
      } else {
        ...
        console.log('\n🔧 Suggestions:');
        console.log(
          '   1. Run "claude-remote setup-permissions" to check permissions'
        );
        console.log('   2. Ensure Claude Code is running in foreground');
        console.log(
          '   3. Try manually clicking input box in Claude Code first'
        );
      }
    } catch (error) {
      console.error('❌ Error occurred during test:', error.message);
