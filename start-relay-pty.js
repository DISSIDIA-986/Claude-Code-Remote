...
function checkConfig() {
  ...
  console.log(
    `🔒 Whitelist senders: ${
      process.env.ALLOWED_SENDERS || '(Not set, will accept all emails)'
    }`
  );
  console.log(
    `💾 Session storage path: ${
      process.env.SESSION_MAP_PATH || '(Using default path)'
    }`
  );
  console.log('');
}
  ...
  if (!fs.existsSync(sessionMapPath)) {
    ...
    const exampleSession = {
      [exampleToken]: {
        ...
        cwd: process.cwd(),
        description:
          'Test session - Include [Claude-Code-Remote #TEST123] in email subject when sending',
      },
    };
    ...
    console.log(
      '   When sending test email, include in subject: [Claude-Code-Remote #TEST123]'
    );
    console.log('');
  }
}
...
function checkSingleInstance() {
  if (fs.existsSync(PID_FILE)) {
    try {
      ...
      console.error(
        '❌ Error: relay-pty service is already running (PID: ' + oldPid + ')'
      );
      console.log(
...
function showInstructions() {
  ...
  console.log(
    '1. When executing tasks in Claude Code, reminder emails containing Token will be sent'
  );
  ...
  console.log('     ```');
  console.log(
    '4. System will automatically extract commands and inject them into corresponding Claude Code session'
  );
...
function main() {
  ...
  console.log(
    '║      Email Command Relay Service - node-pty based PTY mode          ║'
  );
  console.log('╚══════════════════════════════════════════════════════════╝\n');
